...
function calcularPromedio(listaDeNumeros) {
    let suma = 0;
    for (let nota = 0;  nota < listaDeNumeros.length; nota++) {
        suma += listaDeNumeros[nota];
    }
...
function filtrarSoloPositivos(listaDeNumeros) {
    return listaDeNumeros.filter(numeroPositivo => numeroPositivo > 0);
}
...
function contarAprobados(listaDeNotas) {
    let contador = 0;
    for (let nota = 0; nota < listaDeNotas.length; nota++) {
